...
bool CClient::Connect()
{
	...
	param.m_enableAutoConnectionRecovery = true;
	param.m_protocolVersion = g_Version;
	param.m_serverPort = g_ServerPort;
	param.m_serverIP = StringA2T(g_World->m_serverAddr.getCString());

	return m_netClient->Connect(param);
}

// 매 프레임마다의 처리
// Processing every framess
...
{
	CCString txt;
	txt.initWithFormat("Failed to connect server.\n%S", info->ToString().GetString());
	...
	if (info->m_errorType == ErrorType_Ok)
	{
		...
		m_C2SProxy.RequestLocalHeroSpawn(
			HostID_Server, RmiContext::ReliableSend,
			StringA2T(m_localHero->m_name.getCString()), 
			Proud::Vector3(m_localHero->m_localHeroSprite->getPositionX(), 0, m_localHero->m_localHeroSprite->getPositionY()),
			m_localHero->m_direction);
	}
	...
	else
	{
		...
		else
		{
			...
			connectsuccessString.initWithFormat("Sub Client %d succeed to connect server.", m_netClient->GetLocalHostID());
			g_World->Log(connectsuccessString.getCString());
		}
	}
}
...
void CClient::OnServerOffline(CRemoteOfflineEventArgs &args)
{

}

void CClient::OnServerOnline(CRemoteOnlineEventArgs &args)
{
...
CCString CClient::GetHeroName()
{
	CCString ret;
	ret.initWithFormat("%s-%d", g_World->m_userName.getCString(), m_ID);
	...
	if (remoteHero)
	{
		...
		if (oldValue != remoteHero->m_synchedThruDirectP2P)
		{
			...
			if (IsUnderUserControl())
			{
				...
				CCString logString;
				logString.initWithFormat("Remote Client %s started sending a message directly to P2P", remoteHero->m_name.getCString());
				g_World->Log(logString.getCString());
			}
		}
...
DEFRMI_SynchWorldS2C_RemoteHero_Appear(CClient)
{
	...
	if (remote != m_netClient->GetLocalHostID())
	{
		...
		if (!remoteHero)
		{
			...
			StringA name = StringT2A(heroName.GetString());
			remoteHero->m_name = name.GetString();
		...
		remoteHero->m_directionFollower.SetFollowerAngleVelocity(M_PI / 180*60);
		...
		if (IsUnderUserControl())
		{
			...
			CCString logString;
			logString.initWithFormat("Remote Client %S appeared on your sight", heroName.GetString());
			g_World->Log(logString.getCString());
		}
	}
	return true;
...
DEFRMI_SynchWorldS2C_RemoteHero_Disappear(CClient)
{
	...
	if (IsUnderUserControl())
	{
		...
		g_World->Log("Remote client disappeared on your sight");
	}
	return true;
}
...
{
// 	CCString txt;
// 	txt.initWithFormat("%s occured at %s", StringT2A(errorInfo->ToString().GetString()), Proud::StringW2A(__FUNCTIONW__));
// 	GotoErrorForm(txt.getCString());
}

void CClient::OnException(Exception &e)
{
// 	CCString txt;
// 	txt.initWithFormat("%s occured at %s", e.what(), Proud::StringW2A(__FUNCTIONW__));
// 	GotoErrorForm(txt.getCString());
}

void CClient::GotoErrorForm(const char* text)
{
	g_World->GoToErrorScene(text);
}

StringA CClient::GetLocalAddress()
{
	AddrPort addr = m_netClient->GetTcpLocalAddr();
	return StringT2A(addr.ToString());
}
