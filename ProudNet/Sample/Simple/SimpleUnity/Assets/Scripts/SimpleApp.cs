	...
	public class SimpleApp : MonoBehaviour
	{
		...
		IEnumerator Start()
		{
			...
			else
			{
				...
				m_simpleProxy.Chat(HostID.HostID_Server, RmiContext.ReliableSend, "접속 직후 채팅 메세지.", 22, 22.33f);
			}
		...
		void OnGUI()
		{
			...
			// 스크롤바가 그려질 영역이 끝났음을 의미.
			GUI.EndScrollView();

			if (GUI.Button(new Rect(m_screenWidth / 2 - m_screenOneW, m_screenOneH * 8, m_screenOneW, m_screenOneH), "Send"))
			{
                m_simpleProxy.Chat(HostID.HostID_Server, RmiContext.ReliableSend, "서버로의 채팅 메세지.", 22, 22.33f);
			}
			...
			if (GUI.Button(new Rect(m_screenWidth / 2, m_screenOneH * 8, m_screenOneW, m_screenOneH), "P2PSend"))
			{
                if (m_eventSink.GroupID != HostID.HostID_Server)
					m_simpleProxy.P2PChat(m_eventSink.GroupID, RmiContext.ReliableSend, "P2P 채팅 메세지.", 1, 1);
			}
